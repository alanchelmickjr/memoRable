...
export class ContextIntegrationService {
   ...
   */
  async setDeviceContext(context: DeviceContextFrame): Promise<void> {
    ...
    await this.redis.sAdd(
      DEVICE_REDIS_KEYS.activeDevices(context.userId),
      context.deviceId
    );
  ...
  async getDeviceContext(
    userId: string,
    deviceId: string
  ): Promise<DeviceContextFrame | null> {
   ...
   */
  async getAllDeviceContexts(userId: string): Promise<DeviceContextFrame[]> {
    const deviceIds = await this.redis.sMembers(
      DEVICE_REDIS_KEYS.activeDevices(userId)
    );

    const contexts: DeviceContextFrame[] = [];
   ...
   */
  async clearDeviceContext(userId: string, deviceId: string): Promise<void> {
  ...
  async getUnifiedContext(userId: string): Promise<UnifiedUserContext | null> {
    ...
    if (cached) {
      const context = JSON.parse(cached) as UnifiedUserContext;
   ...
   */
  async recomputeUnifiedContext(userId: string): Promise<UnifiedUserContext | null> {
    ...
    const unified: UnifiedUserContext = {
      userId,
    ...
    await this.redis.setEx(
      ...
      Math.floor(STALENESS_CONFIG.unifiedContextRefreshInterval / 1000),
  ...
  private resolveLocation(
    contexts: DeviceContextFrame[]
  ): UnifiedUserContext['location'] {
    ...
    const best = otherContexts.reduce((a, b) =>
      b.location!.confidence > a.location!.confidence ? b : a
    );
  ...
  private resolvePeople(
    contexts: DeviceContextFrame[]
  ): UnifiedUserContext['people'] {
    const allPeople = new Map<string, { confidence: number; sources: string[] }>();
    const sourceMap = new Map<string, string[]>();

    for (const ctx of contexts) {
      if (!ctx.people) continue;
  ...
  private detectPatterns(
    contexts: DeviceContextFrame[],
    activeDevices: UnifiedUserContext['activeDevices']
  ): UnifiedUserContext['patterns'] {
    ...
    const primaryDevice = nonStale.length > 0
      ? nonStale.reduce((a, b) =>
          new Date(b.lastUpdate).getTime() > new Date(a.lastUpdate).getTime() ? b : a
        ).deviceId
   ...
   */
  async getContextForRetrieval(userId: string): Promise<{
    location: string;
    activity: string;
    people: string[];
    deviceContext: {
    ...
    const primaryDevice = unified.activeDevices.find(
      (d) => d.deviceId === unified.patterns.primaryDevice
    );
...
// ============================================================================
// Redis Client Type (for type safety)
// ============================================================================
...
let integrationServiceInstance: ContextIntegrationService | null = null;

export function getContextIntegrationService(
...
export function resetContextIntegrationService(): void {
