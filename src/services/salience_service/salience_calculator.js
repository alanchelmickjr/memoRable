 ...
 */
import { DEFAULT_SALIENCE_WEIGHTS, CONTEXT_WEIGHT_MODIFIERS, } from './models';
 ...
 */
export function computeSalience(features, context, config = DEFAULT_CONFIG) {
    ...
    // Weighted sum
    const rawScore = components.emotional * weights.emotional +
 ...
 */
function calculateComponents(features, context, userProfile) {
    return {
 ...
 */
function calculateEmotionalScore(features) {
    let score = 0;
 ...
 */
function calculateNoveltyScore(features, context, userProfile) {
    let score = 0;
    // New people (not in known contacts)
    if (userProfile?.knownContacts) {
        for (const person of features.peopleMentioned) {
            const isKnown = userProfile.knownContacts.some((c) => c.toLowerCase() === person.toLowerCase());
            if (!isKnown) {
    ...
    if (userProfile?.recentTopics && features.topics.length > 0) {
        const novelTopics = features.topics.filter((topic) => !userProfile.recentTopics.some((recent) => recent.toLowerCase().includes(topic.toLowerCase())));
 ...
 */
function calculateRelevanceScore(features, userProfile) {
    let score = 0;
    // User name mentioned (if we have it)
    if (userProfile?.name) {
        const nameMentioned = features.peopleMentioned.some((p) => p.toLowerCase() === userProfile.name.toLowerCase());
        if (nameMentioned) {
            score += 30;
        }
    }
    ...
    if (userProfile?.interests && features.topics.length > 0) {
        const matchingTopics = features.topics.filter((topic) => userProfile.interests.some((interest) => interest.toLowerCase().includes(topic.toLowerCase()) ||
    ...
    if (userProfile?.closeContacts && features.peopleMentioned.length > 0) {
        const closeOnes = features.peopleMentioned.filter((person) => userProfile.closeContacts.some((close) => close.toLowerCase() === person.toLowerCase()));
        score += Math.min(40, closeOnes.length * 20);
    }
    // Goals mentioned
    if (userProfile?.goals && features.topics.length > 0) {
        const goalRelated = features.topics.filter((topic) => userProfile.goals.some((goal) => goal.toLowerCase().includes(topic.toLowerCase()) ||
    ...
    if (!userProfile) {
        ...
        const selfActions = features.actionItems.filter((item) => item.assignedTo === 'self' || item.assignedTo === 'mutual');
 ...
 */
function calculateSocialScore(features) {
    let score = 0;
 ...
 */
function calculateConsequentialScore(features) {
    let score = 0;
 ...
 */
function applyContextModifiers(baseWeights, modifiers = {}) {
    const adjusted = {
 ...
 */
export function calculateDecayModifier(daysSinceCapture, config = DEFAULT_CONFIG) {
 ...
 */
export function calculateRetrievalBoost(currentScore, retrievalCount) {
...
export function detectContext(context) {
    ...
    if (context.timeBucket === 'work_hours' &&
        context.dayOfWeek &&
 ...
 */
export function buildCaptureContext(timestamp = new Date(), location, isLocationNew) {
 ...
 */
function checkNearHoliday(date) {
 ...
 */
export async function getLearnedWeights(userId) {
    const now = Date.now();
    // Check cache first
 ...
 */
export function invalidateWeightsCache(userId) {
 ...
 */
export async function computeSalienceForUser(features, context, userId, userProfile) {
