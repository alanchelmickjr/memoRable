...
export async function createOpenLoopsFromFeatures(
  ...
  for (const commitment of features.commitments) {
    const loop = createLoopFromCommitmentSync(
      commitment,
      userId,
      memoryId,
      memoryCreatedAt,
  ...
  for (const request of features.requestsMade) {
    const loop = createLoopFromRequestSync(
      request,
      userId,
      memoryId,
      memoryCreatedAt,
  ...
  for (const agreement of features.mutualAgreements) {
    const loop = createLoopFromAgreementSync(
      agreement,
      userId,
      memoryId,
      memoryCreatedAt,
  ...
  for (const question of features.questionsAsked) {
    const loop = createLoopFromQuestion(
      question,
      userId,
      memoryId,
      memoryCreatedAt
...
function createLoopFromCommitmentSync(
  ...
  // Parse due date
  const { dueDate, softDeadline } = parseDueDate(
    commitment.byWhen,
    commitment.dueType,
    createdAt
  );
  ...
  return {
    ...
    memoryId,
    loopType:
      commitment.type === "made" ? "commitment_made" : "commitment_received",
...
function createLoopFromAgreementSync(
  ...
  if (!contact) return null;
  // Parse timeframe
  const { dueDate, softDeadline } = parseTimeframe(
    agreement.timeframe,
    createdAt
  );
  const urgency = determineUrgency(
    dueDate,
    agreement.specificity === "specific" ? "explicit" : "implicit"
  );
  return {
    id: uuidv4(),
    userId,
...
function parseDueDate(byWhen, dueType, referenceDate) {
  ...
  if (lowerDate.includes("tomorrow")) {
    ...
    return dueType === "explicit"
      ? { dueDate: tomorrow }
      : { softDeadline: tomorrow };
  }
...
export async function closeLoop(loopId, completedMemoryId) {
  await collections.openLoops().updateOne(
    { id: loopId },
    {
      $set: {
        status: "completed",
...
export async function checkLoopClosures(
  ...
  for (const person of features.peopleMentioned) {
    ...
    for (const loop of openLoops) {
      ...
      if (llmClient) {
        const result = await checkClosureWithLLM(
          newMemoryText,
          loop.description,
          llmClient
        );
...
function withTimeout(promise, timeoutMs, fallback) {
  return Promise.race([
    ...
    new Promise((resolve) => {
      setTimeout(() => {
        console.warn(
          `[OpenLoopTracker] LLM call timed out after ${timeoutMs}ms`
        );
        resolve(fallback);
      }, timeoutMs);
    }),
...
export async function markReminderSent(loopId) {
  ...
  nextReminder.setDate(
    nextReminder.getDate() + Math.max(1, loop.escalateAfterDays / 2)
  );
  await collections.openLoops().updateOne(
    { id: loopId },
    {
