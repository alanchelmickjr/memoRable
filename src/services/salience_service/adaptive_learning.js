 ...
 */
export async function recalibrateWeights(userId, config = DEFAULT_CONFIG) {
 ...
 */
function blend(base, learned, rate) {
 ...
 */
async function saveLearnedWeights(weights) {
    await collections.learnedWeights().updateOne({ userId: weights.userId }, { $set: weights }, { upsert: true });
 ...
 */
export async function getEffectiveWeights(userId, config = DEFAULT_CONFIG) {
 ...
 */
export async function markRetrievalAction(retrievalLogId, actionType) {
    await collections.retrievalLogs().updateOne({ id: retrievalLogId }, {
        $set: {
 ...
 */
export async function recordFeedback(retrievalLogId, feedback) {
    await collections.retrievalLogs().updateOne({ id: retrievalLogId }, { $set: { userFeedback: feedback } });
}
/**
 * Get insights about user's memory patterns.
 */
export async function getMemoryPatternInsights(userId) {
    const learned = await getLearnedWeights(userId);
 ...
 */
function determineTrend(logs) {
    if (logs.length < 5)
        return 'insufficient_data';
    // Sort by date
    const sorted = [...logs].sort((a, b) => new Date(a.retrievedAt).getTime() - new Date(b.retrievedAt).getTime());
    ...
    if (Math.abs(diff) < 5)
        return 'stable';
    if (diff > 0)
        return 'increasing_salience';
    return 'decreasing_salience';
}
/**
 * Reset learned weights to defaults.
 */
export async function resetWeights(userId) {
 ...
 */
export async function batchRecalibrateWeights() {
    ...
    for (const userId of activeUsers) {
        try {
            const result = await recalibrateWeights(userId);
