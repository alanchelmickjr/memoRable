...
import { getOpenLoops, getOverdueLoops, getUpcomingDueLoops, type OpenLoopWithOverdue } from './open_loop_tracker';
...
export interface EnergyAwareTask {
  ...
  energyMatch: number; // 0-1 score for current energy
  ...
  triagePriority: number; // 1-5 (1 = highest)
...
export type TriageCategory =
  | 'do_now'        // Urgent + fits energy
  | 'schedule'      // Important but needs right energy
  | 'delegate'      // Could be handed off
  | 'quick_win'     // Low effort, easy completion
  | 'deep_work'     // Requires sustained focus
  | 'batch'         // Group with similar tasks
  | 'defer'         // Not urgent, save for later
  | 'reconsider';   // May not be worth doing
 ...
 */
const URGENCY_PRIORITY: Record<LoopUrgency, number> = {
  urgent: 1,
  high: 2,
  normal: 3,
  low: 4,
 ...
 */
export async function getEnergyAwareTasks(
  userId: string,
  context: EnergyContext
): Promise<EnergyAwareTaskResult> {
  ...
  if (context.preferCategories && context.preferCategories.length > 0) {
    relevantLoops = allLoops.filter((loop) =>
      context.preferCategories!.includes(loop.category)
    );
  }
  ...
  const energyTasks = relevantLoops.map((loop) =>
    assessTaskForEnergy(loop, context)
  );

  // Categorize by energy match
  const recommended: EnergyAwareTask[] = [];
  const quickWins: EnergyAwareTask[] = [];
  const deferredForHigherEnergy: EnergyAwareTask[] = [];
  ...
  const overdueTasks = overdueLoops.map((loop) =>
    assessTaskForEnergy(loop, context)
  );
 ...
 */
export async function getDeepWorkTasks(
  userId: string,
  ...
  return result.recommended.filter(
    (task) =>
 ...
 * @param energyLevel - Current energy level
 */
export function batchTriageTasks(
  loops: OpenLoop[],
  energyLevel: EnergyLevel = 'medium'
): EnergyAwareTask[] {
...
function assessTaskForEnergy(
  ...
  const triageCategory = determineTriageCategory(
    loop,
    cognitiveLoad,
    urgencyScore,
    energyMatch,
  ...
  const triageRecommendation = generateTriageRecommendation(
    loop,
    triageCategory,
    context.currentEnergy,
    energyMatch
 ...
 */
function upgradeLoad(load: CognitiveLoad): CognitiveLoad {
 ...
 */
function downgradeLoad(load: CognitiveLoad): CognitiveLoad {
  const levels: CognitiveLoad[] = ['minimal', 'light', 'moderate', 'heavy', 'intense'];
  const index = levels.indexOf(load);
 ...
 */
function getOptimalEnergyLevel(load: CognitiveLoad, complexity: TaskComplexity): EnergyLevel {
...
function determineTriageCategory(
  ...
  if (
    (cognitiveLoad === 'minimal' || cognitiveLoad === 'light') &&
 ...
 */
export function getCognitiveLoadDescription(load: CognitiveLoad): string {
 ...
 */
export function suggestEnergyForTimeOfDay(
  hour: number = new Date().getHours()
): EnergyLevel {
  // Typical energy patterns (adjust based on user data if available)
  if (hour >= 9 && hour <= 11) return 'peak';     // Morning peak
  if (hour >= 14 && hour <= 16) return 'medium';  // Post-lunch dip
  if (hour >= 16 && hour <= 18) return 'high';    // Afternoon recovery
  if (hour >= 6 && hour <= 8) return 'high';      // Early morning
