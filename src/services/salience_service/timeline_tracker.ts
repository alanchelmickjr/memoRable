 ...
 */
const RELATIONSHIP_EVENT_MAPPING: Record<
  RelationshipEventType,
  { type: PersonEventType; sensitivity: EventSensitivity; goodToMention: boolean }
> = {
 ...
 */
async function createEventFromDate(
  date: ExtractedDate,
  userId: string,
  memoryId: string,
  createdAt: Date
): Promise<PersonTimelineEvent | null> {
  if (!date.whose || !date.context) return null;
 ...
 */
async function createEventFromRelationship(
  personName: string,
  relEvent: RelationshipEventType,
  userId: string,
  memoryId: string,
  createdAt: Date
): Promise<PersonTimelineEvent | null> {
 ...
 */
function inferEventTypeFromContext(
  context: string
): { type: PersonEventType; sensitivity: EventSensitivity; goodToMention: boolean } {
 ...
 */
function isRecurringEvent(context: string): boolean {
  const lower = context.toLowerCase();
 ...
 */
function getRemindBeforeDays(type: PersonEventType, sensitivity: EventSensitivity): number {
  if (sensitivity === 'sensitive') return 0; // Don't proactively remind
 ...
 */
export async function getUpcomingEventsForContact(
  userId: string,
  contactId: string,
  daysAhead: number = 14
): Promise<PersonTimelineEvent[]> {
 ...
 */
export async function getRecentEventsForContact(
  userId: string,
  contactId: string,
  daysBack: number = 30
): Promise<PersonTimelineEvent[]> {
 ...
 */
export async function getEventsNeedingReminders(userId: string): Promise<PersonTimelineEvent[]> {
  const now = new Date();
  ...
  return events.filter((event) => {
    if (!event.eventDate || !event.remindBeforeDays) return false;
 ...
 */
export async function markEventMentioned(eventId: string): Promise<void> {
  await collections.personTimelineEvents().updateOne(
    { id: eventId },
    {
      $set: {
 ...
 */
export async function getSensitiveContext(
  userId: string,
  contactId: string
): Promise<PersonTimelineEvent[]> {
 ...
 */
export async function processRecurringEvents(userId: string): Promise<number> {
  const now = new Date();
