 ...
 */
export async function extractFeatures(text, llmClient, currentDate = new Date()) {
...
function transformToExtractedFeatures(response) {
    return {
        ...
        relationshipEvents: (response.relationship_events || []).filter(isValidRelationshipEvent),
 ...
 */
function transformActionItem(item) {
    return {
 ...
 */
function transformCommitment(item) {
    return {
 ...
 */
function transformAgreement(item) {
    return {
        what: item.what || '',
 ...
 */
function isValidRelationshipEvent(event) {
    const validEvents = [
 ...
 */
export function extractFeaturesHeuristic(text) {
    ...
    // Relationship events
    if (/died|passed away|funeral|death/i.test(text))
        features.relationshipEvents.push('death');
    if (/married|wedding|engaged/i.test(text))
        features.relationshipEvents.push('marriage');
    if (/divorced|separation/i.test(text))
        features.relationshipEvents.push('divorce');
    if (/pregnant|baby|born|birth/i.test(text))
        features.relationshipEvents.push('birth');
    if (/promoted|promotion/i.test(text))
        features.relationshipEvents.push('promotion');
    if (/graduated|graduation/i.test(text))
        features.relationshipEvents.push('graduation');
    if (/sick|illness|hospital|cancer/i.test(text))
        features.relationshipEvents.push('illness');
