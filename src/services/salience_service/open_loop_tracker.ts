 ...
 */
function createLoopFromQuestion(
  question: string,
  userId: string,
  memoryId: string,
  createdAt: Date
 ...
 */
function getEscalationDays(urgency: LoopUrgency): number {
  switch (urgency) {
 ...
 */
function categorizeCommitment(description: string): LoopCategory {
...
export async function getOpenLoops(
  ...
  if (options.contactName) query.otherParty = new RegExp(`^${options.contactName}$`, 'i');
...
async function checkClosureWithLLM(
  ...
  llmClient: LLMClient
): Promise<LoopClosureCheckResponse> {
 ...
 */
export async function getOverdueLoops(userId: string): Promise<OpenLoop[]> {
  const now = new Date();
 ...
 */
export async function getUpcomingDueLoops(
  userId: string,
  daysAhead: number = 3
): Promise<OpenLoop[]> {
  const now = new Date();
 ...
 */
export async function markReminderSent(loopId: string): Promise<void> {
 ...
 */
export async function abandonLoop(loopId: string, reason?: string): Promise<void> {
  await collections.openLoops().updateOne(
    { id: loopId },
    {
      $set: {
