...
import { getUpcomingEventsForContact, getRecentEventsForContact, getSensitiveContext, } from './timeline_tracker';
import { getRelationshipPattern, getLatestSnapshot, } from './relationship_tracker';
 ...
 */
export async function generateBriefing(userId, contactId, options = {}) {
    const opts = { ...DEFAULT_OPTIONS, ...options };
    // Get contact info
    const contact = await collections.contacts?.().findOne({ _id: contactId });
    const contactName = contact?.name || 'Unknown';
    // Fetch all data in parallel (consolidated open loops query: 1 DB call instead of 3)
    const [pattern, snapshot, allOpenLoops, upcomingEvents, recentEvents, sensitiveContext,] = await Promise.all([
    ...
    const keyMemories = await getHighSalienceMemories(userId, contactId, opts.minSalience, opts.maxMemories);
    ...
    const suggestions = opts.includeSuggestions
        ? generateSuggestions(openLoopsYouOwe, upcomingEvents, sensitiveContext, recentEvents)
 ...
 */
function buildRelationshipInfo(contact, pattern) {
    return {
 ...
 */
async function getHighSalienceMemories(userId, contactId, minSalience, limit) {
 ...
 */
async function getRecentMemories(userId, contactId, limit) {
    try {
 ...
 */
function calculateRecentSentiment(memories) {
 ...
 */
function generateSuggestions(loopsYouOwe, upcomingEvents, sensitiveContext, recentEvents) {
    const topics = [];
    const sensitivities = [];
    ...
    const deathEvents = [...sensitiveContext, ...recentEvents].filter((e) => e.description.toLowerCase().includes('death') ||
 ...
 */
function getDueDateUrgency(loop) {
 ...
 */
export async function generateQuickBriefing(userId, contactId) {
    const contact = await collections.contacts?.().findOne({ _id: contactId });
 ...
 */
export async function generateBriefingByName(userId, contactName, options) {
