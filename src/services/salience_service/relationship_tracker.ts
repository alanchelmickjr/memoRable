 ...
 */
function updateTypicalDays(current: string[], newDay: string): string[] {
 ...
 */
function updateTypicalTimes(current: string[], newTime: string): string[] {
 ...
 */
function calculateTrend(pattern: RelationshipPattern, currentGap: number | null): EngagementTrend {
 ...
 */
export async function getRelationshipPattern(
  userId: string,
  contactId: string
): Promise<RelationshipPattern | null> {
  ...
  if (pattern && pattern.lastInteraction) {
    ...
    const daysSince = Math.ceil(
      (Date.now() - new Date(pattern.lastInteraction).getTime()) / (1000 * 60 * 60 * 24)
    );
 ...
 */
export async function getColdRelationships(
  userId: string,
  options: {
  ...
  return patterns.map((p) => {
    if (p.lastInteraction) {
      p.daysSinceLastInteraction = Math.ceil(
        (now.getTime() - new Date(p.lastInteraction).getTime()) / (1000 * 60 * 60 * 24)
      );
    }
    return p;
  });
 ...
 */
export async function getDecreasingRelationships(userId: string): Promise<RelationshipPattern[]> {
  ...
  return patterns.filter((p) => {
    if (!p.lastInteraction || !p.avgDaysBetweenInteractions) return false;
 ...
 */
export async function createRelationshipSnapshot(
  userId: string,
  contactId: string,
  openLoops: OpenLoop[],
  avgSentiment?: number
  ...
  const favorsGiven = openLoops.filter(
    (l) => l.owner === 'self' && l.category === 'favor' && l.status === 'completed'
  ).length;
  const favorsReceived = openLoops.filter(
    (l) => l.owner === 'them' && l.category === 'favor' && l.status === 'completed'
  ).length;
  ...
  await collections.relationshipSnapshots().updateOne(
    { userId, contactId, snapshotDate },
    { $set: snapshot },
    { upsert: true }
  );
 ...
 */
export async function getRelationshipTrajectory(
  userId: string,
  contactId: string,
  daysBack: number = 90
): Promise<RelationshipSnapshot[]> {
...
export async function getRelationshipsNeedingAttention(
  ...
  withOverdueLoops: Array<{ pattern: RelationshipPattern; overdueCount: number }>;
}> {
  ...
  const overdueByContact = new Map<string, number>();
  for (const loop of allLoops) {
    if (loop.contactId) {
      overdueByContact.set(
        loop.contactId,
        (overdueByContact.get(loop.contactId) || 0) + 1
      );
    }
  }

  // Get patterns for contacts with overdue loops
  const withOverdueLoops: Array<{ pattern: RelationshipPattern; overdueCount: number }> = [];
 ...
 */
export async function setNudgeThreshold(
  userId: string,
  contactId: string,
  days: number
): Promise<void> {
  await collections.relationshipPatterns().updateOne(
    { userId, contactId },
    {
      $set: {
 ...
 */
export async function getRelationshipsDueForNudge(userId: string): Promise<RelationshipPattern[]> {
  const patterns = await collections.relationshipPatterns()
    .find({
      userId,
  ...
  return patterns.filter((p) => {
    if (!p.lastInteraction || !p.nudgeIfGapExceedsDays) return false;
 ...
 */
export async function updateAllSnapshots(userId: string): Promise<number> {
