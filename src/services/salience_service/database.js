 ...
 */
export async function setupSalienceDatabase(database) {
    db = database;
...
export const collections = {
    memories: () => getCollection('memories'),
    openLoops: () => getCollection('open_loops'),
    personTimelineEvents: () => getCollection('person_timeline_events'),
    relationshipPatterns: () => getCollection('relationship_patterns'),
    relationshipSnapshots: () => getCollection('relationship_snapshots'),
    retrievalLogs: () => getCollection('retrieval_logs'),
    learnedWeights: () => getCollection('learned_weights'),
    contacts: () => getCollection('contacts'),
    stateChanges: () => getCollection('state_changes'),
};
/**
 * Get or create a contact by name.
 * Sanitizes name input for security.
 */
export async function getOrCreateContact(userId, name, metadata) {
    // Sanitize name first
    ...
    if (existing) {
        // Update lastSeenAt
        await contactsCollection.updateOne({ _id: existing._id }, {
            $set: {
 ...
 */
export async function batchGetOrCreateContacts(userId, names) {
    const contactsCollection = getCollection('contacts');
    const result = new Map();
    // Sanitize and filter names - deduplicate by sanitized lowercase
    const sanitizedNames = new Map(); // lowercase -> sanitized
    ...
    if (toCreate.length > 0) {
        const newContacts = toCreate.map(name => ({
            userId,
            name,
            firstSeenAt: now,
            lastSeenAt: now,
        ...
        const insertResult = await contactsCollection.insertMany(newContacts);
    ...
    if (existing.length > 0) {
        await contactsCollection.updateMany({ _id: { $in: existing.map(c => c._id) } }, { $set: { lastSeenAt: now, updatedAt: now } });
    }
    return result;
}
/**
 ...
 */
export async function addSalienceIndexesToMemories(memoriesCollection) {
    const indexes = [
    ...
    for (const index of indexes) {
        try {
            await memoriesCollection.createIndex(index.spec);
        }
        catch (error) {
            if (error.code !== 85 && error.code !== 86) {
