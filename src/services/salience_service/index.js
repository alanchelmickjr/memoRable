...
export { createOpenLoopsFromFeatures, getOpenLoops, closeLoop, checkLoopClosures, getOverdueLoops, getUpcomingDueLoops, markReminderSent, abandonLoop, } from './open_loop_tracker';
...
export { retrieveWithSalience, logRetrieval, boostOnRetrieval, timeAwareRetrieve, getMemoriesForPerson, getMemoriesWithUpcomingDeadlines, } from './retrieval';
...
export { initContextFrame, getContextFrame, updateContextFrame, clearContextFrame, addPersonToFrame, removePersonFromFrame, surfaceMemoriesForFrame, setContext, whatMattersNow, 
// Multi-device support
getAllDeviceContexts, getUnifiedUserContext, clearDeviceContext, } from './context_frame';
// Re-export device context types for multi-device architecture
export { DeviceRegistry, DEVICE_REDIS_KEYS, STALENESS_CONFIG, createDefaultDeviceContext, } from './device_context';
...
export { RealtimeSyncService, ContextHub, getSyncService, getContextHub, resetSyncServices, SYNC_CHANNELS, SENSOR_PRIORITY, } from './realtime_sync';
// Re-export memory operations (forget, reassociate, export)
export { forgetMemory, forgetPerson, restoreMemory, reassociateMemory, mergeMemories, exportMemories, importMemories, } from './memory_operations';
// Re-export anticipation service (predictive memory surfacing)
export { initAnticipationService, observeContext, recordPatternFeedback, getAnticipatedContext, getPatternStats, generateDayAnticipation, THRESHOLDS as ANTICIPATION_THRESHOLDS, WINDOWS as ANTICIPATION_WINDOWS, } from './anticipation_service';
// Re-export energy-aware task retrieval (TaskForge Triage integration)
export { getEnergyAwareTasks, getQuickWins, getDeepWorkTasks, getTasksForTimeBlock, triageTask, batchTriageTasks, getEnergyDescription, getCognitiveLoadDescription, suggestEnergyForTimeOfDay, } from './energy_aware_tasks';
// Re-export LLM provider abstraction (Bedrock, Anthropic, OpenAI)
export { createLLMClient, BedrockLLMClient, AnthropicLLMClient, OpenAILLMClient, } from './llm_providers';
...
import { startTimer, incMemoriesProcessed, incOpenLoopsCreated, incOpenLoopsClosed, incTimelineEvents, incErrors, observeProcessingTime, observeSalienceScore, } from './metrics';
 ...
 * Time: ~200-500ms
 */
export async function enrichMemoryWithSalience(input, llmClient, userProfile) {
    ...
    try {
        ...
        const salience = await computeSalienceForUser(features, context, input.userId, userProfile);
        ...
        const [openLoops, timelineEvents] = await Promise.all([
            ...
            createOpenLoopsFromFeatures(features, input.userId, input.memoryId, memoryCreatedAt),
            // Step 5: Create timeline events for other people
            createTimelineEventsFromFeatures(features, input.userId, input.memoryId, memoryCreatedAt),
        ...
        const closedLoopIds = await checkLoopClosures(input.text, features, input.userId, input.memoryId, llmClient);
        // Record metrics
 ...
 * Time: ~10-50ms
 */
export async function enrichMemoryWithSalienceHeuristic(input, userProfile) {
    const endTimer = startTimer();
    try {
        const memoryCreatedAt = new Date();
 ...
 */
export async function backfillSalience(memories, llmClient, options = {}) {
    ...
    for (let i = 0; i < memories.length; i += batchSize) {
        ...
        const results = await Promise.allSettled(batch.map((memory) => enrichMemoryWithSalience({
            memoryId: memory.id,
            text: memory.text,
 ...
 */
export async function getSalienceStatus(userId) {
    ...
    const [youOweLops, theyOweLoops, mutualLoops, overdueLoops, upcomingEvents, activeRelationships, coldRelationships, weights,] = await Promise.all([
 ...
 */
export async function runDailyMaintenance(userId) {
