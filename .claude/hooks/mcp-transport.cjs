 ...
 *
 * The MCP server may be:
 * - Local standalone (server.js on port 8080)
 * - EC2 deployment (Elastic IP:8080)
 * - Docker (memorable_mcp_server on port 8080)
 *
...
function mcpInit() {
  try {
    ...
    if (response && (response.result || response.id)) {
      ...
      // Complete handshake — notifications/initialized (fire and forget)
      try {
...
function mcpCall(toolName, args) {
  ...
  try {
    ...
    // Unwrap MCP content array → parsed object
